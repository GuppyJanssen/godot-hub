...
func _ready() -> void:
	...
	my_id = "BasicSpawner_" + str(int(global_position.x)) + "_0_" + str(int(global_position.y)) + "_0"
...
func spawn_single_enemy() -> void:
	...
	var enemy_base_name = "BasicEnemy_" + str(int(global_position.x)) + "_0_" + str(int(global_position.y)) + "_0"
	
	if total_enemies_to_spawn == 1:
		inst.name = enemy_base_name
	else:
		inst.name = enemy_base_name + "_grunt_" + str(enemies_spawned_count)
